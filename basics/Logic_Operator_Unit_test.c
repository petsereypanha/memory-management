...
munit_case(SUBMIT, test_premium_overrides, {
  ...
  munit_assert_int(
      result, ==, 1,
      "Premium status should grant access regardless of other factors");
});

munit_case(SUBMIT, test_no_access, {
  int result = can_access_registry(0, 50, 0);
  munit_assert_int(result, ==, 0,
                   "Should not have access without meeting any criteria");
});
